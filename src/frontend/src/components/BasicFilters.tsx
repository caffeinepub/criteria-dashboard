import { Button } from "@/components/ui/button";
import { Checkbox } from "@/components/ui/checkbox";
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from "@/components/ui/select";
import { Info, RotateCcw, Save } from "lucide-react";
import { useState } from "react";
import { toast } from "sonner";

interface BasicFiltersProps {
  mode: "users" | "groups";
}

const ORDER_STATUS_OPTIONS = [
  { value: "RECEIVED", label: "RECEIVED" },
  { value: "RE-PROCESS", label: "RE-PROCESS" },
  { value: "BATCH ORDERS", label: "BATCH ORDERS" },
];

function RequiredLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-0.5 mb-1">
      <span
        className="text-[11px] font-semibold"
        style={{ color: "oklch(0.35 0.05 245)" }}
      >
        {children}
      </span>
      <span
        className="text-[11px] font-bold"
        style={{ color: "oklch(0.52 0.22 25)" }}
      >
        *
      </span>
    </div>
  );
}

function OptionalLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-1 mb-1">
      <span
        className="text-[11px] font-semibold"
        style={{ color: "oklch(0.35 0.05 245)" }}
      >
        {children}
      </span>
    </div>
  );
}

export default function BasicFilters({ mode }: BasicFiltersProps) {
  const [branch, setBranch] = useState("");
  const [group, setGroup] = useState("");
  const [users, setUsers] = useState("");
  const [product, setProduct] = useState("");
  const [productExclude, setProductExclude] = useState(false);
  const [client, setClient] = useState("");
  const [clientExclude, setClientExclude] = useState(false);
  const [stateVal, setStateVal] = useState("");
  const [stateExclude, setStateExclude] = useState(false);
  const [county, setCounty] = useState("");
  const [countyExclude, setCountyExclude] = useState(false);
  const [orderStatus, setOrderStatus] = useState<string[]>([]);
  const [subStatus, setSubStatus] = useState("");

  const toggleStatus = (val: string) => {
    setOrderStatus((prev) =>
      prev.includes(val) ? prev.filter((s) => s !== val) : [...prev, val],
    );
  };

  const handleReset = () => {
    setBranch("");
    setGroup("");
    setUsers("");
    setProduct("");
    setProductExclude(false);
    setClient("");
    setClientExclude(false);
    setStateVal("");
    setStateExclude(false);
    setCounty("");
    setCountyExclude(false);
    setOrderStatus([]);
    setSubStatus("");
    toast("Filters reset");
  };

  const handleSave = () => toast.success("Criteria saved successfully");

  return (
    <div
      className="bg-white rounded-xl overflow-hidden"
      style={{
        border: "1px solid oklch(0.88 0.01 240)",
        boxShadow: "0 1px 4px oklch(0 0 0 / 0.06)",
      }}
      data-ocid="basic_filters.panel"
    >
      {/* Note bar */}
      <div
        className="flex items-center gap-2 px-4 py-2"
        style={{
          background: "oklch(0.96 0.03 240)",
          borderBottom: "1px solid oklch(0.90 0.02 240)",
        }}
      >
        <Info
          className="w-3.5 h-3.5 shrink-0"
          style={{ color: "oklch(0.48 0.18 220)" }}
        />
        <span
          className="text-[11px] font-medium"
          style={{ color: "oklch(0.38 0.10 220)" }}
        >
          Note: Select At least one product, client, state
        </span>
      </div>

      <div className="px-4 py-3 space-y-3">
        {/* Row 1: Required + Optional dropdowns */}
        <div className="flex flex-wrap gap-x-4 gap-y-2 items-end">
          {/* Branch */}
          <div className="flex flex-col">
            <RequiredLabel>Branch</RequiredLabel>
            <Select value={branch} onValueChange={setBranch}>
              <SelectTrigger
                className="h-8 text-xs w-28"
                data-ocid="basic_filters.branch.select"
              >
                <SelectValue placeholder="Select Branch" />
              </SelectTrigger>
              <SelectContent>
                {["BLR", "MUM", "DEL", "HYD", "CHE"].map((b) => (
                  <SelectItem key={b} value={b}>
                    {b}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>

          {/* Group */}
          <div className="flex flex-col">
            <RequiredLabel>Group</RequiredLabel>
            <Select value={group} onValueChange={setGroup}>
              <SelectTrigger
                className="h-8 text-xs w-32"
                data-ocid="basic_filters.group.select"
              >
                <SelectValue placeholder="Select Group" />
              </SelectTrigger>
              <SelectContent>
                {["MTPROCESS", "ALPHA", "BETA", "GAMMA"].map((g) => (
                  <SelectItem key={g} value={g}>
                    {g}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>

          {/* Users / SubGroup */}
          <div className="flex flex-col">
            <RequiredLabel>
              {mode === "users" ? "Users" : "SubGroup"}
            </RequiredLabel>
            <Select value={users} onValueChange={setUsers}>
              <SelectTrigger
                className="h-8 text-xs w-36"
                data-ocid="basic_filters.users.select"
              >
                <SelectValue
                  placeholder={
                    mode === "users" ? "Select Users" : "Select SubGroup"
                  }
                />
              </SelectTrigger>
              <SelectContent>
                {mode === "users" ? (
                  <>
                    <SelectItem value="ravi">Ravi Kumar</SelectItem>
                    <SelectItem value="priya">Priya Nair</SelectItem>
                    <SelectItem value="arun">Arun Singh</SelectItem>
                    <SelectItem value="amrutha">Amrutha</SelectItem>
                    <SelectItem value="abhishek">Abhishek D</SelectItem>
                  </>
                ) : (
                  <>
                    <SelectItem value="sg1">SG-Alpha</SelectItem>
                    <SelectItem value="sg2">SG-Beta</SelectItem>
                    <SelectItem value="sg3">SG-Gamma</SelectItem>
                  </>
                )}
              </SelectContent>
            </Select>
          </div>

          {/* Product + Exclude */}
          <div className="flex flex-col">
            <div className="flex items-center gap-2 mb-1">
              <span
                className="text-[11px] font-semibold"
                style={{ color: "oklch(0.35 0.05 245)" }}
              >
                Product
              </span>
              <div className="flex items-center gap-1 cursor-pointer">
                <Checkbox
                  checked={productExclude}
                  onCheckedChange={(v) => setProductExclude(!!v)}
                  className="w-3 h-3"
                  data-ocid="basic_filters.product_exclude.checkbox"
                />
                <span
                  className="text-[10px]"
                  style={{ color: "oklch(0.52 0.03 240)" }}
                >
                  Exclude
                </span>
              </div>
            </div>
            <Select value={product} onValueChange={setProduct}>
              <SelectTrigger
                className="h-8 text-xs w-40"
                data-ocid="basic_filters.product.select"
              >
                <SelectValue placeholder="Select Product" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="10yr">10 YR SEARCH</SelectItem>
                <SelectItem value="20yr">20 YR SEARCH</SelectItem>
                <SelectItem value="rundown">Run Down</SelectItem>
                <SelectItem value="other">Other</SelectItem>
              </SelectContent>
            </Select>
          </div>

          {/* Client + Exclude */}
          <div className="flex flex-col">
            <div className="flex items-center gap-2 mb-1">
              <span
                className="text-[11px] font-semibold"
                style={{ color: "oklch(0.35 0.05 245)" }}
              >
                Client
              </span>
              <div className="flex items-center gap-1 cursor-pointer">
                <Checkbox
                  checked={clientExclude}
                  onCheckedChange={(v) => setClientExclude(!!v)}
                  className="w-3 h-3"
                  data-ocid="basic_filters.client_exclude.checkbox"
                />
                <span
                  className="text-[10px]"
                  style={{ color: "oklch(0.52 0.03 240)" }}
                >
                  Exclude
                </span>
              </div>
            </div>
            <Select value={client} onValueChange={setClient}>
              <SelectTrigger
                className="h-8 text-xs w-40"
                data-ocid="basic_filters.client.select"
              >
                <SelectValue placeholder="Select Client" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="c1">180 Real Estate</SelectItem>
                <SelectItem value="c2">Avenue 365 Lender</SelectItem>
                <SelectItem value="c3">Client C</SelectItem>
              </SelectContent>
            </Select>
          </div>

          {/* State + Exclude */}
          <div className="flex flex-col">
            <div className="flex items-center gap-2 mb-1">
              <span
                className="text-[11px] font-semibold"
                style={{ color: "oklch(0.35 0.05 245)" }}
              >
                State
              </span>
              <div className="flex items-center gap-1 cursor-pointer">
                <Checkbox
                  checked={stateExclude}
                  onCheckedChange={(v) => setStateExclude(!!v)}
                  className="w-3 h-3"
                  data-ocid="basic_filters.state_exclude.checkbox"
                />
                <span
                  className="text-[10px]"
                  style={{ color: "oklch(0.52 0.03 240)" }}
                >
                  Exclude
                </span>
              </div>
            </div>
            <Select value={stateVal} onValueChange={setStateVal}>
              <SelectTrigger
                className="h-8 text-xs w-36"
                data-ocid="basic_filters.state.select"
              >
                <SelectValue placeholder="Select State" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="AL">Alabama</SelectItem>
                <SelectItem value="TX">Texas</SelectItem>
                <SelectItem value="MD">Maryland</SelectItem>
                <SelectItem value="CA">California</SelectItem>
              </SelectContent>
            </Select>
          </div>

          {/* County + Exclude */}
          <div className="flex flex-col">
            <div className="flex items-center gap-2 mb-1">
              <span
                className="text-[11px] font-semibold"
                style={{ color: "oklch(0.35 0.05 245)" }}
              >
                County
              </span>
              <div className="flex items-center gap-1 cursor-pointer">
                <Checkbox
                  checked={countyExclude}
                  onCheckedChange={(v) => setCountyExclude(!!v)}
                  className="w-3 h-3"
                  data-ocid="basic_filters.county_exclude.checkbox"
                />
                <span
                  className="text-[10px]"
                  style={{ color: "oklch(0.52 0.03 240)" }}
                >
                  Exclude
                </span>
              </div>
            </div>
            <Select value={county} onValueChange={setCounty}>
              <SelectTrigger
                className="h-8 text-xs w-36"
                data-ocid="basic_filters.county.select"
              >
                <SelectValue placeholder="Select County" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="autauga">Autauga</SelectItem>
                <SelectItem value="baldwin">Baldwin</SelectItem>
                <SelectItem value="montgomery">Montgomery</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>

        {/* Row 2: Order Status + Sub Status */}
        <div className="flex flex-wrap gap-x-6 gap-y-2 items-start">
          {/* Order Status */}
          <div className="flex flex-col">
            <RequiredLabel>Order Status</RequiredLabel>
            <div
              className="flex items-center gap-4 px-3 py-2 rounded-lg"
              style={{ border: "1px solid oklch(0.85 0.08 262)" }}
            >
              {ORDER_STATUS_OPTIONS.map((opt) => (
                <div
                  key={opt.value}
                  className="flex items-center gap-1.5 cursor-pointer"
                >
                  <Checkbox
                    checked={orderStatus.includes(opt.value)}
                    onCheckedChange={() => toggleStatus(opt.value)}
                    className="w-3.5 h-3.5"
                    data-ocid={`basic_filters.status_${opt.value.toLowerCase().replace(/[^a-z0-9]+/g, "_")}.checkbox`}
                  />
                  <span
                    className="text-xs font-medium"
                    style={{ color: "oklch(0.35 0.05 245)" }}
                  >
                    {opt.label}
                  </span>
                </div>
              ))}
            </div>
          </div>

          {/* Sub Status */}
          <div className="flex flex-col">
            <OptionalLabel>Sub Status</OptionalLabel>
            <Select value={subStatus} onValueChange={setSubStatus}>
              <SelectTrigger
                className="h-8 text-xs w-36"
                data-ocid="basic_filters.sub_status.select"
              >
                <SelectValue placeholder="Select Sub Status" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="pending">Pending</SelectItem>
                <SelectItem value="inprogress">In Progress</SelectItem>
                <SelectItem value="review">Under Review</SelectItem>
                <SelectItem value="copies">CopiesDownload</SelectItem>
                <SelectItem value="developer">DeveloperSearch</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>

        {/* Row 3: Actions */}
        <div className="flex items-center justify-end gap-2 pt-1">
          <Button
            variant="ghost"
            size="sm"
            className="h-8 text-xs gap-1.5"
            style={{ color: "oklch(0.50 0.05 240)" }}
            onClick={handleReset}
            data-ocid="basic_filters.reset.button"
          >
            <RotateCcw className="w-3 h-3" />
            Reset
          </Button>
          <Button
            size="sm"
            className="h-8 text-xs text-white border-0 gap-1.5"
            style={{ background: "oklch(0.48 0.22 262)" }}
            onClick={handleSave}
            data-ocid="basic_filters.save.button"
          >
            <Save className="w-3 h-3" />
            Save
          </Button>
        </div>
      </div>

      {/* Bottom note */}
      <div
        className="flex items-center gap-2 px-4 py-2"
        style={{
          background: "oklch(0.98 0.01 55)",
          borderTop: "1px solid oklch(0.93 0.03 55)",
        }}
      >
        <Info
          className="w-3.5 h-3.5 shrink-0"
          style={{ color: "oklch(0.52 0.18 55)" }}
        />
        <span className="text-[10px]" style={{ color: "oklch(0.40 0.08 55)" }}>
          Note: If the status is &quot;Hold&quot;, it indicates that the
          criteria is currently on hold and cannot be used for processing
          orders.
        </span>
      </div>
    </div>
  );
}
